...
def load_movie(movie_file):
    movie = stable_retro.Movie(movie_file)
    duration = -1
    while movie.step():
        duration += 1
    movie = stable_retro.Movie(movie_file)
    movie.step()
    emulator = stable_retro.make(
        game=movie.get_game(),
...
def main(argv=sys.argv[1:]):
    ...
    stable_retro.data.add_integrations(stable_retro.data.Integrations.ALL)

    if args.csv_out:
        m0 = stable_retro.Movie(args.movies[0])
        game = m0.get_game()
        reward_fields = (
