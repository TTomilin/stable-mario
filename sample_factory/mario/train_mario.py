...
from sample_factory.mario.mario_utils import MARIO_ENVS, make_mario_env, MarioSpec
...
def add_multi_envs(cfg):
    if cfg.game_list != None:
        name = ",".join(sorted(cfg.game_list))
        id = "{0}-v0".format(name)
        MARIO_ENVS.append(MarioSpec(name, id))
