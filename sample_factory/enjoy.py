...
def save_video(frames, file_path):
    writer = imageio.get_writer(file_path, fps=35)
    for frame in frames:
...
def enjoy(cfg: Config) -> Tuple[StatusCode, float]:
    ...
    env_initials = ''.join(word[0].upper() for word in cfg.game.split('_'))
    ...
    checkpoints = PPOLearner.get_checkpoints(PPOLearner.checkpoint_dir(cfg, policy_id), f"{name_prefix}_*")
    checkpoint_dict = PPOLearner.load_checkpoint(checkpoints, device)
