...
class NoopResetEnv(gym.Wrapper):
    """
    Sample initial states by taking random number of no-ops on reset for MultiBinary action spaces.
    No-op is assumed to be an action of all zeros in the multi-binary action space.
    ...
    def __init__(self, env: gym.Env, noop_max: int = 30):
        ...
        # Check if it's MultiBinary and set noop_action accordingly
        if isinstance(env.action_space, gym.spaces.MultiBinary):
            self.noop_action = np.zeros(env.action_space.n, dtype=int)  # MultiBinary: all zeros
        else:
            self.noop_action = 0  # Default to 0 for Discrete, adjust based on your needs

        # You can handle a fallback here if needed
        if hasattr(env.unwrapped, 'get_action_meanings'):
            assert env.unwrapped.get_action_meanings()[0] == "NOOP", "Environment should support NOOP as action 0."
